...
class LiDARApp:
    ...
    def run_detection(self):
        """Run the real-time detection loop with improved precision"""
        try:
            scan_counter = 0
            last_update_time = time.time()

            # Отслеживание касаний с улучшенной точностью
            tracked_touches = {}
            frame_counter = 0
            ...
            smoothing_window = config.precision['smoothing_window']
            
            # Параметры синхронизации
            sync_tolerance = config.precision['sync_tolerance']
            
            while self.running:
                current_scan = self.lidar.get_last_scan()
        
                if current_scan and len(current_scan) > 0:
                    # Синхронизация начала скана (0° ± tolerance)
                    ...
                    # Детектируем точки касания с повышенной точностью
                    all_touch_points = self.detector.detect_touch_points(corrected_scan)
            
                    # Расширенная фильтрация
                    real_touch_points = []
                    for point in all_touch_points:
                        # Множественная проверка на шум и валидность
                        if (self._is_valid_touch_point(point) and 
                            self._passes_multiple_filters(point)):
                            real_touch_points.append(point)
            
                    # Точки внутри рамки проекции с улучшенной проверкой
                    frame_touch_points = self._get_precise_touch_points(real_touch_points)
                    
                    # СГЛАЖИВАНИЕ ДВИЖЕНИЯ МЫШИ
                    if (self.mouse_controller and self.mouse_controller.is_active and 
                        frame_touch_points):
                        
                        # Используем только одну точку для максимальной точности
                        primary_touch = frame_touch_points[0] if frame_touch_points else None
                        
                        if primary_touch:
                            touch_key = "primary"
                            
                            # Добавляем точку в историю
                            if touch_key not in mouse_position_history:
                                mouse_position_history[touch_key] = []
                            
                            mouse_position_history[touch_key].append({
                                'point': primary_touch,
                                'time': time.time()
                            })
                            
                            # Ограничиваем размер истории
                            if len(mouse_position_history[touch_key]) > smoothing_window:
                                mouse_position_history[touch_key].pop(0)
                            
                            # Вычисляем сглаженную позицию
                            smoothed_point = self._smooth_touch_point(
                                mouse_position_history[touch_key]
                            )
                            
                            if smoothed_point:
                                try:
                                    # Перемещаем мышь в сглаженную позицию
                                    self.mouse_controller.move_mouse_to_touch(smoothed_point)
                                except Exception as e:
                                    self.log_message(f"⚠️ Mouse movement error: {e}")
                    
                    # Обновление графика с оптимизированной частотой
                    current_time = time.time()
                    if current_time - last_update_time > config.precision['mouse_update_rate']:
                        self.update_plot(corrected_scan, frame_touch_points[:3])  # Только первые 3 точки
    ...
    def _is_valid_touch_point(self, point: Point) -> bool:
        """Проверка точки касания на валидность"""
        # Минимальная интенсивность
        if point.intensity < config.precision['min_touch_intensity']:
            return False
        
        # Максимальное расстояние (защита от дальних ложных срабатываний)
        if point.distance > config.precision['max_detection_distance']:
            return False
        
        return True

    def _passes_multiple_filters(self, point: Point) -> bool:
        """Множественная фильтрация точки"""
        # Проверка постоянного шума
        if (self.persistent_noise_manager and 
            self.persistent_noise_manager.is_persistent_noise(point)):
            return False
        
        # Проверка профильного шума
        if (self.noise_filter and 
            self.noise_filter.is_false_positive(point)):
            return False
        
        return True

    def _get_precise_touch_points(self, touch_points: List[Point]) -> List[Point]:
        """Получение точек касания с улучшенной точностью"""
        if not touch_points:
            return []
        
        # Фильтруем точки внутри проекции
        filtered_points = []
        if self.corners:
            ...
            filtered_points = touch_points[:5]  # Ограничиваем количество
        
        # Сортируем по интенсивности (наиболее уверенные первыми)
        filtered_points.sort(key=lambda p: p.intensity, reverse=True)
        
        return filtered_points[:3]  # Только 3 самые уверенные точки

    def _smooth_touch_point(self, history: List[dict]) -> Point:
        """Сглаживание позиции касания"""
        if not history:
            return None
        ...
        # Взвешенное среднее (более свежие точки имеют больший вес)
        total_weight = 0
        weighted_angles = 0
        ...
        for i, record in enumerate(history):
            weight = i + 1  # Вес увеличивается с каждым новым элементом
            point = record['point']
            
            total_weight += weight
            weighted_angles += point.angle * weight
            weighted_distances += point.distance * weight
            weighted_intensities += point.intensity * weight
        
        if total_weight == 0:
            return history[-1]['point']
        
        smoothed_point = Point(
            angle=weighted_angles / total_weight,
            distance=weighted_distances / total_weight,
            intensity=weighted_intensities / total_weight
        )
    ...
    def update_plot(self, scan=None, touch_points=None):
        ...
        try:
            ...
            self.ax.set_theta_zero_location('N')
            self.ax.set_theta_direction(-1)
            self.ax.set_title("LiDAR Scan (Precision Mode)")
            ...
            if touch_points:
                ...
                self.ax.scatter(np.radians(touch_angles), touch_distances, 
                            c='red', s=100, alpha=1.0, marker='o', label='Touch Points')
                
                # Выделяем основную точку другим цветом для лучшей видимости
                if len(touch_points) > 0:
                    primary_point = touch_points[0]
                    self.ax.scatter(np.radians(primary_point.angle), primary_point.distance,
                                  c='lime', s=80, alpha=1.0, marker='*', 
                                  label='Primary Touch Point', edgecolors='white', linewidth=2)
